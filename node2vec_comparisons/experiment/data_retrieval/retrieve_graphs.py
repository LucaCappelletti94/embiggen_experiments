...
def retrieve_coo_wikipedia() -> Graph:
    ...
    urls = compress_json.local_load("internet_archive_urls.json")
    url = urls["EnglishWikipedia"]

    downloader = BaseDownloader(verbose=2)
    downloader.download(url)

    numpy_edge_list = "downloads/wikipedia_edge_list.npy"
    tsv_edge_list = "downloads/wikipedia_edge_list.tsv"

    if not os.path.exists(tsv_edge_list):
        edge_list = np.load(numpy_edge_list).T
        pd.DataFrame(
            edge_list,
